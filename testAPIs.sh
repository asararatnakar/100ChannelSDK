...
echo
curl -s -X POST \
  http://localhost:4000/channels/mychannel$i/chaincodes \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json" \
  -d "{
...
TRX_ID=$(curl -s -X POST \
  http://localhost:4000/channels/mychannel${i}/chaincodes/mycc \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json" \
  -d '{
...
echo
curl -s -X GET \
  "http://localhost:4000/channels/mychannel${i}/chaincodes/mycc?peer=peer0.org1.example.com&fcn=query&args=%5B%22a%22%5D" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
echo
echo
